#!/usr/bin/env python3
"""
Split scanned IOL PDFs using OCR to detect problem boundaries
"""
...
def find_problem_boundaries_ocr(pdf_file):
    """
    Use OCR to find problem boundaries in scanned PDFs
    Returns: dict mapping problem number to start page (0-indexed)
    """
    print(f"  Using OCR to analyze scanned PDF...")
...
def split_pdf(pdf_file, output_dir, year):
    """
    Split a scanned IOL PDF into individual problem files
    """
    print(f"\nProcessing: {pdf_file}")
    print(f"Year: {year}")

    # Find problem boundaries using OCR
    problem_pages = find_problem_boundaries_ocr(pdf_file)

    if not problem_pages:
        print("  ⚠️  No problem markers found via OCR!")
        return False

    if len(problem_pages) < 5:
        print(f"  ⚠️  Only found {len(problem_pages)}/5 problems, may need manual page ranges")
...
def main():
    # Scanned years
    years = [2003, 2004, 2005, 2007, 2008, 2009, 2014]

    base_dir = "/Users/skyliu/Lingohub/olympiad-problems/IOL/by-year"

    print("=" * 80)
    print("IOL OCR PDF Splitter - Processing scanned PDFs")
    print("=" * 80)

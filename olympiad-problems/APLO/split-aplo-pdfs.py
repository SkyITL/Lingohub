#!/usr/bin/env python3
"""
Automatically split APLO problem PDFs into separate problem files
"""
...
def find_problem_boundaries(pdf_file):
    ...
    for page_num, page in enumerate(reader.pages):
        ...
        # Look for "Problem N" markers (N = 1-5)
        for problem_num in range(1, 6):
            # Match patterns like "Problem 1 (20 points)"
...
def split_pdf(pdf_file, output_dir, year):
    """
    Split a combined APLO PDF into individual problem files
    """
    print(f"\nProcessing: {pdf_file}")
    print(f"Year: {year}")
    ...
    for i, (problem_num, start_page) in enumerate(sorted_problems):
        ...
        # Save the split PDF
        output_file = os.path.join(output_dir, f"aplo-{year}-p{problem_num}.pdf")
        with open(output_file, 'wb') as output:
            writer.write(output)

        page_count = end_page - start_page
        print(f"  ✅ Created aplo-{year}-p{problem_num}.pdf (pages {start_page + 1}-{end_page}, {page_count} page{'s' if page_count > 1 else ''})")

    return True

def main():
    # Years to process (APLO runs 2019-2024)
    years = [2019, 2020, 2021, 2022, 2023, 2024]

    base_dir = "/Users/skyliu/Lingohub/olympiad-problems/APLO"

    print("=" * 80)
    print("APLO PDF Splitter - Splitting individual problems")
    print("=" * 80)
    ...
    for year in years:
        pdf_file = os.path.join(base_dir, f"aplo-{year}-prob.en.pdf")
        ...
        try:
            if split_pdf(pdf_file, base_dir, year):
                success_count += 1
            else:
                failed_count += 1

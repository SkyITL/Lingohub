#!/usr/bin/env python3
"""
Split IOL individual problem PDFs for en-us versions (2009, 2011, 2013-2015)
"""
...
def split_pdf(pdf_file, output_dir, year):
    ...
    if not problem_pages:
        print("  ⚠️  No problem markers found! (May be scanned images)")
        return False
...
def main():
    # Years with en-us or us versions
    years = [2009, 2011, 2013, 2014, 2015, 2025]

    base_dir = "/Users/skyliu/Lingohub/olympiad-problems/IOL/by-year"

    print("=" * 80)
    print("IOL PDF Splitter - Splitting en-us/us versions (2009, 2011, 2013-2015, 2025)")
    print("=" * 80)
    ...
    for year in years:
        ...
        # Try both .en-us.pdf and .us.pdf versions
        pdf_file = os.path.join(year_dir, f"iol-{year}-indiv-prob.en-us.pdf")
        if not os.path.exists(pdf_file):
            pdf_file = os.path.join(year_dir, f"iol-{year}-indiv-prob.us.pdf")
    ...
    print(f"  Failed/Scanned: {failed_count} years")
    if scanned_list:
        print(f"  Scanned images: {scanned_list}")
